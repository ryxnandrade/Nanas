      ...
      <Layout>
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            ...
            <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">Erro ao carregar carteiras</h2>
      ...
      <div className="space-y-8">
        ...
        >
          <div>
            ...
            <p className="text-muted-foreground mt-2 text-lg">Gerencie e acompanhe todas as suas contas e saldos</p>
          </div>
        ...
        <AnimatedCard
          delay={0.1}
          className="bg-gradient-to-br from-primary via-primary/80 to-primary/60 text-white border-0 shadow-2xl"
        >
          <AnimatedCardContent>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-primary-foreground/90 text-sm font-medium opacity-90">
                  Saldo Total em Todas as Contas
                </p>
                <p className="text-5xl font-bold mt-3 text-balance">{formatCurrency(getTotalBalance())}</p>
                <p className="text-primary-foreground/80 text-sm mt-3">
                  {carteiras.length} carteira{carteiras.length !== 1 ? "s" : ""} ativa
                  {carteiras.length !== 1 ? "s" : ""}
                </p>
              </div>
          ...
          <AnimatedCard delay={0.2}>
            <AnimatedCardContent>
              <div className="text-center py-16">
                ...
                <h3 className="text-2xl font-bold text-foreground mb-2">Nenhuma carteira encontrada</h3>
                ...
                return (
                  ...
                  >
                    <AnimatedCard
                      hover={true}
                      delay={0}
                      className={cn("relative overflow-hidden group", colors.border)}
                    >
                      <div
                        className={cn(
                          "absolute top-0 right-0 w-32 h-32 opacity-5 rounded-full blur-3xl group-hover:opacity-10 transition-opacity duration-300",
                          colors.bg,
                        )}
                      />

                      <AnimatedCardHeader className="relative z-10">
                        <div className="flex items-start justify-between">
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              <AnimatedCardTitle className="text-lg font-bold">{carteira.nome}</AnimatedCardTitle>
                      ...
                      <AnimatedCardContent className="relative z-10">
                        <div className="space-y-4">
                          <div>
                            <p className="text-sm text-muted-foreground mb-2 font-medium">Saldo Atual</p>
                            <p
                              className={cn(
                                "text-3xl font-bold",
                                (carteira.saldo || 0) >= 0
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>Detalhes da Carteira: {selectedWallet?.nome}</DialogTitle>
              <DialogDescription>Visualize as informações desta carteira</DialogDescription>
            </DialogHeader>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <p className="text-lg font-semibold">{selectedWallet.tipo}</p>
                  </div>
                  <div className="space-y-2">
                ...
                {selectedWallet.descricao && (
                  <div>
                    <p className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">Descrição</p>
                    <p className="text-gray-900 dark:text-white">{selectedWallet.descricao}</p>
                  </div>
                )}
              </div>
            )}
